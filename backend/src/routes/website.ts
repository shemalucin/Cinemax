...
authRouter.get("/api/comments/movie/:movieId", (req, res) => {
  ...
  try {
    ...
    const movieComments = allComments
      .filter((c: any) => c.movieId === movieIdNum)
      .sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

    // Enrich comments with user information
    const enrichedComments = movieComments.map((comment: any) => {
      const user = db.data.users.find((u: any) => u.id === comment.userId);
      return {
        ...comment,
...
// Image recognition endpoint for visual search (public endpoint)
    ...
    if (openaiApiKey) {
      try {
        ...
        if (data.choices && data.choices[0]?.message?.content) {
          const movieName = data.choices[0].message.content.trim();
