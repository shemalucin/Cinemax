...
// Every outbound AI provider call gets wrapped with this so a slow/hanging
// provider can't stall the whole request — it fails fast and the chain
// below moves on to the next key instead of leaving the user waiting.
const AI_PROVIDER_TIMEOUT_MS = 15000;

async function withTimeout<T>(promise: Promise<T>, label: string, ms: number = AI_PROVIDER_TIMEOUT_MS): Promise<T> {
...
// Tries all four configured AI providers in order, automatically moving to
// the next one the moment a call errors out, times out, or the key is
// missing/exhausted (rate-limited, out of quota, etc). Whichever key
// happens to be unset simply throws "not configured" immediately and the
// chain skips it — so this works correctly whether you've set 1, 2, 3, or
// all 4 of GROK_API_KEY / OPENAI_API_KEY / GEMINI_API_KEY / GROQ_API_KEY.
// Order: Grok (xAI) -> OpenAI -> Gemini -> Groq.
async function routedAssistantChat(messages: Array<{ role: string; content: string }>, groqModel?: string): Promise<{ text: string; engine: "openai" | "grok" | "gemini" | "groq" }> {
  const chain: Array<{ engine: "grok" | "openai" | "gemini" | "groq"; run: () => Promise<string> }> = [
