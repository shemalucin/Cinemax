...
var dataDir, DB_PATH, AppStateSchema, AppStateModel, usingMongo, data, ready, saveInFlight, savePending, db, db_default;
var init_db = __esm({
  "src/lib/db.ts"() {
    dataDir = path.join(process.cwd(), "data");
    DB_PATH = path.join(dataDir, "cinemax.json");
    AppStateSchema = new mongoose.Schema(
      { _id: { type: String, default: "singleton" }, state: mongoose.Schema.Types.Mixed },
      { minimize: false, versionKey: false, strict: false }
    );
    AppStateModel = mongoose.models.AppState || mongoose.model("AppState", AppStateSchema, "app_state");
    usingMongo = false;
    data = emptySchema();
    ready = false;
    saveInFlight = null;
...
var COOKIE_NAME, TOKEN_EXPIRY, TOKEN_EXPIRY_MS, JWT_SECRET, DEFAULT_PREFERENCES, DISPOSABLE_DOMAINS, OTP_TTL_MS, OTP_RESEND_COOLDOWN_MS, OTP_MAX_ATTEMPTS, otpStore, signupVerifyStore, passwordResetStore, rateBuckets, ACTIVE_WINDOW_MS, lastSeenAt;
var init_auth = __esm({
  "src/lib/auth.ts"() {
    init_db();
    COOKIE_NAME = "cinemax_session";
    TOKEN_EXPIRY = "7d";
    ...
    DISPOSABLE_DOMAINS = /* @__PURE__ */ new Set([
      "mailinator.com",
      "guerrillamail.com",
      "tempmail.com",
      "10minutemail.com",
    ...
    OTP_RESEND_COOLDOWN_MS = 60 * 1e3;
    OTP_MAX_ATTEMPTS = 5;
    otpStore = /* @__PURE__ */ new Map();
    signupVerifyStore = /* @__PURE__ */ new Map();
    passwordResetStore = /* @__PURE__ */ new Map();
...
var BREVO_API_KEY, FROM_EMAIL, configured;
var init_mailer = __esm({
  "src/lib/mailer.ts"() {
    BREVO_API_KEY = (process.env.BREVO_API_KEY || "").trim();
    FROM_EMAIL = "cinemaxmov01@gmail.com";
    configured = false;
    if (BREVO_API_KEY) {
      configured = true;
...
function registerChatClient(id, res) {
  sseClients.set(id, { id, res });
}
function unregisterChatClient(id) {
...
function broadcastChatEvent(event, payload) {
...
function clearPresence(clientId) {
...
function setTyping(entry, ttlMs = 3500) {
  cleanupTyping();
...
function clearTypingForActor(actorId) {
  let changed = false;
  for (const [key, value] of typingEntries.entries()) {
...
function pushChatActivity(icon, text) {
...
var sseClients, presenceEntries, typingEntries, activityFeed;
var init_chatRealtime = __esm({
  "src/lib/chatRealtime.ts"() {
    sseClients = /* @__PURE__ */ new Map();
    presenceEntries = /* @__PURE__ */ new Map();
    typingEntries = /* @__PURE__ */ new Map();
    activityFeed = [];
    setInterval(() => {
      cleanupTyping();
      broadcastChatEvent("presence", { entries: getPresenceEntries() });
...
function normalizeSharedMovie(raw) {
  ...
  return {
    ...
    overview: raw.overview ? String(raw.overview) : void 0
  };
}
function normalizedReactions(raw) {
...
function toggleReaction(reactions, emoji, actorId) {
...
function getChatActor(req) {
  ...
  return {
    actorId: guestId || `guest-${crypto3.randomUUID()}`,
    userId: "",
    guestId: guestId || `guest-${crypto3.randomUUID()}`,
    name: "Guest Viewer",
    avatar: "",
    isGuest: true
  };
}
function buildChatMeta(viewerId) {
  const online = getPresenceEntries();
  const todayStart = /* @__PURE__ */ new Date();
  todayStart.setHours(0, 0, 0, 0);
  const todayIso = todayStart.toISOString();
  const messagesToday = db_default.data.chat_messages.filter((m) => m.created_at >= todayIso).length + db_default.data.direct_messages.filter((m) => m.created_at >= todayIso).length;
  const nowWatching = online.filter((entry) => !!entry.currentMovieTitle).length;
  const trendingCounts = /* @__PURE__ */ new Map();
  for (const item of db_default.data.watch_history.filter((h) => h.watched_at >= todayIso && h.title)) {
  ...
  const trendingMovieEntry = Array.from(trendingCounts.entries()).sort((a, b) => b[1].count - a[1].count)[0] || ["Community Favorite", { count: 0, poster: null }];
  const roomStats = CHAT_ROOMS.map((room) => ({
    ...room,
    messageCount: db_default.data.chat_messages.filter((m) => (m.room_id || "action") === room.id).length
  }));
  const topMembers = Array.from(
    db_default.data.chat_messages.reduce((map, msg) => {
  ...
  ).sort((a, b) => b.reputation - a.reputation).slice(0, 5).map((member, index) => ({
    ...member,
    level: Math.max(1, Math.round(member.messages / 5) + 1),
    badge: index === 0 ? "Movie Master" : member.messages > 15 ? "Top Critic" : "Film Fan"
  }));
  const onlineMembers = online.filter((entry) => entry.userId && entry.userId !== viewerId).slice(0, 8).map((entry) => ({
    userId: entry.userId,
    name: entry.name,
    avatar: entry.avatar || "",
    status: entry.status,
  ...
  return {
    ...
    stats: {
      ...
      reviewsPosted: db_default.data.comments.filter((c) => c.created_at >= todayIso).length,
    ...
    rooms: roomStats,
    onlineMembers,
    watchParties: online.filter((entry) => !!entry.currentMovieTitle).slice(0, 5).map((entry) => ({
      id: `${entry.clientId}-${entry.currentMovieTitle}`,
      hostName: entry.name,
      title: entry.currentMovieTitle
    })),
    topMembers,
    topReviews: db_default.data.comments.filter((c) => c.status === "approved").sort((a, b) => (a.rating || 0) < (b.rating || 0) ? 1 : -1).slice(0, 4).map((c) => ({
      id: c.id,
      author: c.user_name,
...
var LIST_STORAGE_LIMIT_BYTES, DOWNLOAD_STORAGE_LIMIT_BYTES, DEFAULT_ITEM_BYTES, authRouter, CHAT_ROOMS, MAX_MEDIA_DATA_URL_LENGTH;
var init_website = __esm({
  "src/routes/website.ts"() {
    ...
    init_mailer();
    init_chatRealtime();
    LIST_STORAGE_LIMIT_BYTES = 2 * 1024 * 1024 * 1024;
    DOWNLOAD_STORAGE_LIMIT_BYTES = 2 * 1024 * 1024 * 1024;
    DEFAULT_ITEM_BYTES = 150 * 1024 * 1024;
    authRouter = Router();
    ...
    authRouter.post(
      "/api/auth/google",
      rateLimit({ name: "google-auth", max: 10, windowMs: 15 * 60 * 1e3 }),
      async (req, res) => {
        ...
        if (user) {
          ...
          const randomPassword = crypto3.randomBytes(24).toString("hex");
          user = createUser(verified.email, randomPassword, verified.name, void 0, verified.sub, verified.picture);
        }
        const token = signToken(user.id);
        setSessionCookie(res, token);
    ...
    authRouter.get("/api/comments/movie/:movieId", (req, res) => {
      ...
      try {
        const allComments = db_default.data.comments || [];
        const movieComments = allComments.filter((c) => c.movieId === movieIdNum).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        const enrichedComments = movieComments.map((comment) => {
          const user = db_default.data.users.find((u) => u.id === comment.userId);
          return {
            ...comment,
    ...
    authRouter.post("/api/comments", requireAuth, rateLimit({ name: "post-comment", max: 10, windowMs: 15 * 60 * 1e3 }), (req, res) => {
      ...
      if (!userId) {
        res.status(401).json({ error: "You must be signed in to post a comment" });
        return;
      }
      if (!movieId || !content || !content.trim()) {
        res.status(400).json({ error: "Movie ID and content are required" });
        return;
      }
      ...
      try {
        const user = db_default.data.users.find((u) => u.id === userId);
        if (!user) {
        ...
        if (!db_default.data.comments) {
          db_default.data.comments = [];
        }
        const newComment = {
          id: crypto3.randomUUID(),
          userId,
        ...
        db_default.data.comments.push(newComment);
        db_default.save();
    ...
    authRouter.get("/api/auth/age-verification", requireAuth, (req, res) => {
      ...
      if (existing) {
        existing.last_accessed_at = nowIso2;
        ...
        db_default.data.gens_access.push({
          ...
          first_accessed_at: nowIso2,
          last_accessed_at: nowIso2,
          access_count: 1
        });
      }
      db_default.save();
    ...
    MAX_MEDIA_DATA_URL_LENGTH = 35e5;
    authRouter.get("/api/chat/global", (req, res) => {
      const viewerId = getOptionalUserId(req);
      const roomId = String(req.query.roomId || "").trim();
      const messages = db_default.data.chat_messages.filter((m) => !roomId || (m.room_id || "action") === roomId).slice(-500).map((m) => toPublicChatMessage(m, viewerId));
      res.json({ messages });
    });
    authRouter.post("/api/chat/global", requireAuth, (req, res) => {
      ...
      pushChatActivity("\u{1F4AC}", `${req.user.name} posted in ${CHAT_ROOMS.find((room) => room.id === message.room_id)?.label || "Live Chat"}.`);
      if (message.shared_movie?.title) {
        pushChatActivity("\u{1F3AC}", `${req.user.name} shared ${message.shared_movie.title} in chat.`);
      }
    ...
    authRouter.post("/api/chat/global/:id/react", requireAuth, (req, res) => {
      const message = db_default.data.chat_messages.find((m) => m.id === req.params.id);
    ...
    authRouter.patch("/api/chat/global/:id", requireAuth, (req, res) => {
      const message = db_default.data.chat_messages.find((m) => m.id === req.params.id);
      ...
      message.text = text.slice(0, 1e3);
      message.edited_at = (/* @__PURE__ */ new Date()).toISOString();
      db_default.save();
      const publicMessage = toPublicChatMessage(message, req.user.id);
      broadcastChatEvent("global_message_updated", { message: publicMessage });
      res.json({ message: publicMessage });
    ...
    authRouter.get("/api/chat/meta", (req, res) => {
      res.json(buildChatMeta(getOptionalUserId(req)));
    });
    authRouter.get("/api/chat/stream", (req, res) => {
      const clientId = String(req.query.clientId || crypto3.randomUUID());
...
`);
    ...
    authRouter.post("/api/chat/presence", (req, res) => {
      ...
      upsertPresence({
        ...
        guestId: actor.userId ? void 0 : actor.guestId,
        name: actor.name,
        avatar: actor.avatar,
        status: req.body?.status === "away" ? "away" : "online",
        currentView: req.body?.currentView ? String(req.body.currentView) : void 0,
        currentMovieTitle: req.body?.currentMovieTitle ? String(req.body.currentMovieTitle) : void 0,
        lastActiveAt: (/* @__PURE__ */ new Date()).toISOString(),
        panelOpen: !!req.body?.panelOpen,
        language: req.body?.language ? String(req.body.language) : void 0
      });
      res.json({ ok: true, meta: buildChatMeta(actor.userId || void 0) });
    });
    authRouter.post("/api/chat/typing", (req, res) => {
      const actor = getChatActor(req);
      ...
      setTyping({
        ...
        guestId: actor.userId ? void 0 : actor.guestId,
        name: actor.name,
        roomId: req.body?.roomId ? String(req.body.roomId) : void 0,
        targetUserId: req.body?.targetUserId ? String(req.body.targetUserId) : void 0
      });
      res.json({ ok: true });
    });
    ...
    authRouter.get("/api/chat/conversations/:userId", requireAuth, (req, res) => {
      ...
      for (const m of thread) {
        if (m.to_user_id === myId && !m.read) {
          m.read = true;
          m.seen_at = (/* @__PURE__ */ new Date()).toISOString();
          changed = true;
        }
      }
    ...
    authRouter.post("/api/chat/conversations/:userId", requireAuth, (req, res) => {
      ...
      pushChatActivity("\u{1F4E8}", `${req.user.name} sent a direct message.`);
      if (message.shared_movie?.title) {
        pushChatActivity("\u{1F39E}\uFE0F", `${req.user.name} recommended ${message.shared_movie.title} privately.`);
      }
    ...
    authRouter.post("/api/chat/dm/:id/react", requireAuth, (req, res) => {
      const message = db_default.data.direct_messages.find((m) => m.id === req.params.id);
      const emoji = String(req.body?.emoji || "").trim();
      if (!message) {
        res.status(404).json({ error: "Message not found." });
    ...
    authRouter.patch("/api/chat/dm/:id", requireAuth, (req, res) => {
      const message = db_default.data.direct_messages.find((m) => m.id === req.params.id);
      const text = String(req.body?.text || "").trim();
      if (!message) {
        res.status(404).json({ error: "Message not found." });
    ...
    authRouter.delete("/api/chat/dm/:id", requireAuth, (req, res) => {
      const message = db_default.data.direct_messages.find((m) => m.id === req.params.id);
      if (!message) {
        res.status(404).json({ error: "Message not found." });
        return;
      ...
      db_default.data.direct_messages = db_default.data.direct_messages.filter((m) => m.id !== req.params.id);
      db_default.save();
      broadcastChatEvent("direct_message_deleted", { id: req.params.id, participants: [message.from_user_id, message.to_user_id] });
      res.json({ ok: true });
    });
    authRouter.post("/api/visual-search", async (req, res) => {
      try {
        ...
        if (geminiApiKey) {
          try {
            ...
            if (data2.candidates && data2.candidates[0]?.content?.parts) {
              ...
              const movieMatch = text.match(/(?:movie|film|show)[:\s]+([^,.]+)/i);
              if (movieMatch) {
                res.json({ movie: movieMatch[1].trim(), confidence: 0.9 });
                return;
              }
            }
          } catch (error) {
        ...
        if (openaiApiKey) {
          try {
                ...
                messages: [
                  {
                    ...
                    content: [
                      {
                        type: "text",
                        text: "Identify the movie or TV show in this image. Return only the movie/show name."
                      },
                      {
                        type: "image_url",
                        image_url: {
            ...
            const data2 = await response.json();
            if (data2.choices && data2.choices[0]?.message?.content) {
              const movieName = data2.choices[0].message.content.trim();
              res.json({ movie: movieName, confidence: 0.85 });
              return;
            }
          } catch (error) {
        ...
        res.status(500).json({ error: "Failed to identify movie from image" });
      } catch (error) {
...
var adminRouter, UPLOADS_DIR, videoUpload;
...
function buildSystemContext(req, body2) {
  return {
    ...
    // Request context
    request: {
      url: body2.currentUrl || req.headers.referer || "unknown",
      userAgent: body2.userAgent || req.headers["user-agent"] || "unknown",
      detectedLanguage: body2.detectedLanguage || "en",
    ...
    currentPage: body2.currentPage ? {
      name: body2.currentPage.name,
      component: body2.currentPage.component,
      state: body2.currentPage.state || {}
    } : null,
    // Error context (if provided)
    errorContext: body2.error ? {
      message: body2.error.message,
      stack: body2.error.stack,
      type: body2.error.type,
      timestamp: body2.error.timestamp
    } : null
  };
}
...
CURRENT LANGUAGE: ${language === "rw" ? "Kinyarwanda" : language === "fr" ? "French" : "English"}`;
        const { routedAssistantChat: routedAssistantChat2 } = (init_server(), __toCommonJS(server_exports));
        const messages = [
        ...
        const { text, engine } = await routedAssistantChat2(messages);
        res.json({
          text,
          engine,
          language,
...
var router2, reportValidationRules, reports, reports_default;
var init_reports = __esm({
  "src/routes/reports.ts"() {
    ...
    reports = [];
    router2.post("/", reportValidationRules, async (req, res) => {
      try {
        ...
        reports.push(report);
        console.log("\u{1F6A8} NEW REPORT SUBMITTED:", {
          id: report.id,
    ...
    router2.patch("/:id", (req, res) => {
      ...
      reports[reportIndex].updatedAt = (/* @__PURE__ */ new Date()).toISOString();
      res.json({
        success: true,
...
var init_assistantKnowledge = __esm({
  "src/lib/assistantKnowledge.ts"() {
  }
});
...
async function withTimeout(promise, label, ms = AI_PROVIDER_TIMEOUT_MS) {
  let timer;
...
async function routedAssistantChat(messages, groqModel) {
  const chain = [
    { engine: "grok", run: () => withTimeout(grokChat(messages), "Grok") },
    { engine: "openai", run: () => withTimeout(openaiChat(messages), "OpenAI") },
    { engine: "gemini", run: () => withTimeout(geminiChat(messages), "Gemini") },
    { engine: "groq", run: () => withTimeout(groqChat(messages, groqModel), "Groq") }
  ];
  let lastError;
  for (let i = 0; i < chain.length; i++) {
    const { engine, run } = chain[i];
    try {
      const text = await run();
      if (i > 0) {
        console.log(`[assistant] Recovered via ${engine} after ${i} failed provider(s).`);
      }
      return { text, engine };
    } catch (err) {
      lastError = err;
      const nextEngine = chain[i + 1]?.engine;
      console.warn(
        `[assistant] ${engine} failed${nextEngine ? `; falling back to ${nextEngine}` : " (no more providers left)"}:`,
        err?.message || err
      );
    }
  }
...
var app, allowedOrigins, corsOptions, adminDistPath, AI_PROVIDER_TIMEOUT_MS, PORT;
    ...
    app.post("/api/assistant", async (req, res) => {
      try {
        ...
        res.status(missingKey ? 503 : 500).json({
          error: missingKey ? "I'm having trouble connecting right now. Please check your connection and try again." : "I'm having trouble connecting right now. Please check your connection and try again."
        });
      }
    });
