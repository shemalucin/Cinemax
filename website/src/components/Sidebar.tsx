...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
  ...
  const handleNavClick = (viewId: string) => {
    ...
    if (viewId === "live-chat") {
      // Scroll to live chat section on home page
      setActiveGenre(null);
      setActiveGenreName(null);
      setCurrentView("home");
      setIsOpen(false);
      // Scroll to live chat after a short delay to allow view to change
      setTimeout(() => {
        const chatElement = document.getElementById("live-chat-panel");
        if (chatElement) {
  ...
  return (
    <>
      ...
      <aside
        id="sidebar"
        className={`fixed top-0 bottom-0 left-0 z-50 flex w-56 sm:w-64 flex-col border-r border-white/10 bg-gradient-to-b from-[#0d0e12] to-[#0a0a0a] text-neutral-400 transition-transform duration-300 ease-in-out lg:translate-x-0 shadow-2xl ${
      ...
      >
        ...
        <div id="logo-section" className="relative flex h-16 sm:h-20 items-center justify-between px-4 sm:px-6 border-b border-white/5 bg-gradient-to-r from-transparent via-white/[0.02] to-transparent">
          <div 
